...
// require('electron') aquí solo para shell — ipc no hace falta en este módulo.
...
//
//   La categoría wsl-residuos tiene rutas vacías aquí; se rellenan
//   en analizar() con obtenerRutasWslResiduos() (paquetes Store).
// =============================================================

/**
 * Rutas típicas bajo paquetes WSL / distros Linux de la Store donde
 * pueden quedar temporales y cachés. No incluye la raíz de LocalState
 * (ahí está ext4.vhdx / swap.vhdx).
 *
 * @param {string} localAppData - process.env.LOCALAPPDATA
 * @returns {Promise<string[]>}
 */
async function obtenerRutasWslResiduos(localAppData) {
  const rutas = [];
  if (!localAppData) return rutas;

  const packagesDir = path.join(localAppData, "Packages");
  const subdirs = ["temp", "Temp", "cache", "Cache", "logs", "Logs"];

  function esPaqueteWslONombre(nombre) {
    const n = nombre.toLowerCase();
    if (n.startsWith("microsoftcorporationii.windowssubsystemforlinux_"))
      return true;
    if (n.startsWith("canonicalgrouplimited.")) return true;
    if (n.startsWith("thedebianproject.")) return true;
    if (n.startsWith("kalilinux.")) return true;
    if (n.startsWith("opensuse.")) return true;
    if (n.startsWith("suselinux.")) return true;
    if (n.startsWith("fedoraproject.")) return true;
    if (n.startsWith("alpinelinux.")) return true;
    if (n.startsWith("whitewaterfoundry.")) return true;
    return false;
  }

  try {
    const entradas = await fs.readdir(packagesDir, { withFileTypes: true });
    for (const e of entradas) {
      if (!e.isDirectory() || !esPaqueteWslONombre(e.name)) continue;
      const localState = path.join(packagesDir, e.name, "LocalState");
      for (const sub of subdirs) {
        rutas.push(path.join(localState, sub));
      }
    }
  } catch {
    // Sin carpeta Packages o sin permiso
  }

  // WSL1 heredado: temporales bajo lxss (si existe)
  rutas.push(path.join(localAppData, "lxss", "temp"));
  rutas.push(path.join(localAppData, "lxss", "cache"));

  // Sin duplicados ni rutas vacías
...
function obtenerCategorias() {
  ...
  return [
    // ── Archivos temporales del sistema ─────────────────────
    {
      ...
      descripcion:
        "Archivos que Windows crea al instalar programas o durante " +
    ...
    // ── Archivos de registro (.log) ──────────────────────────
    {
      ...
      descripcion:
        "Registros generados por programas para depurar errores. " +
        "Pueden acumularse durante años sin que el usuario lo sepa.",
      rutas: [path.join(LOCALAPP, "Temp"), path.join(WINDIR, "Logs")],
    ...
    // ── Caché de Google Chrome ───────────────────────────────
    {
      ...
      descripcion:
        "Chrome guarda copias de páginas web, imágenes y scripts " +
    ...
    // ── Caché de Microsoft Edge ──────────────────────────────
    {
      ...
      descripcion:
        "Igual que Chrome pero para Edge. Al formatear el PC " +
    ...
    // ── Caché de Mozilla Firefox ─────────────────────────────
    {
      ...
      descripcion:
        'Archivos temporales de Firefox. La carpeta "cache2" ' +
    ...
    // ── Minidumps (volcados de memoria de errores) ───────────
    {
      ...
      descripcion:
        "Cuando un programa se cierra inesperadamente, Windows " +
    ...
    // ── Miniaturas de Windows ────────────────────────────────
    {
      ...
      descripcion:
        "Windows guarda versiones pequeñas de tus imágenes para " +
        "mostrarlas rápido en el Explorador. Se regeneran solos.",
      rutas: [path.join(LOCALAPP, "Microsoft", "Windows", "Explorer")],
    ...
    // ── Residuos de WSL (Windows Subsystem for Linux) ───────
    {
      id: "wsl-residuos",
      nombre: "Residuos de WSL (Subsistema Linux)",
      descripcion:
        "Temporales y cachés bajo los paquetes de la Store de WSL " +
        "y distros Linux (Ubuntu, Debian, etc.). No incluye los discos " +
        "virtuales (.vhdx) de las instalaciones.",
      rutas: [], // Se completa en analizar() con obtenerRutasWslResiduos()
      extensiones: null,
      recursivo: true,
    },
  ];
...
async function analizar(onProgreso) {
  ...
  const rutasWsl = await obtenerRutasWslResiduos(
    process.env.LOCALAPPDATA || "",
  );
  for (const categoria of categorias) {
    if (categoria.id === "wsl-residuos") {
      categoria.rutas = rutasWsl;
      break;
    }
  }
  ...
  for (const categoria of categorias) {
    ...
    const espacioTotal = archivosCategoria.reduce(
      (suma, a) => suma + a.tamano,
      0,
    );
...
async function eliminar(archivos, onProgreso) {
  ...
  for (let i = 0; i < archivos.length; i++) {
    ...
    try {
      // SEGURIDAD: Detectar symlinks antes de eliminar
      // Previene seguir links peligrosos que podrían apuntar a System32
      ...
      // SEGURIDAD: Reintento con backoff exponencial para TOCTOU
      // El archivo podría haber sido eliminado entre el análisis y ahora
      await ejecutarConReintento(
        async () => {
          await shell.trashItem(archivo.ruta);
        },
        { reintentos: 3, delayMs: 50 } // Reintentar hasta 3 veces
      );
      ...
      console.error(
        `[junkCleaner] No se pudo mover a papelera: ${archivo.ruta}`,
        err.message,
      );
      resultado.errores.push({
        nombre: archivo.nombre,
    ...
    try {
      for await (const entrada of flujoDirectorio) {
        ...
        if (entrada.isFile()) {
          ...
          const pasaFiltroCarpeta =
            !filtroCarpeta || nombreLower.includes(filtroCarpeta.toLowerCase());

          if (pasaFiltroCarpeta) {
            pilaDirectorios.push(rutaCompleta);
          }
        }
      }
    } catch (errorLectura) {
